# Topic Coding Codebook (final nr=90)

## Analysis unit
- One row = one **comment** (`comment_id`), not post/user/thread.

## Machine vs human layers
- Machine `topic_id`: BERTopic+HDBSCAN candidate cluster after `reduce_topics(nr=90)`.
...
2. `sample_comments_10` (10 comments per topic; full fields in `topic_comment_samples_final_nr90.csv`)
...
## Agreement
- Two independent coders fill `coder1_sheet_final_nr90.csv` and `coder2_sheet_final_nr90.csv`.
